...
class AuthService {
  static async signUp({ firstName, lastName, email, password }) {
    const isExist = await UserRepository.findByEmailNonActive({
      email,
    });
    console.log(isExist);
    if (isExist) {
    ...
    if (newUser) {
      ...
      // Send mail
      const link = `${process.env.BASE_URL}/api/v1/auth/active/account/${otp.token}`;
  ...
  static activeAccount = async ({ token }) => {
    ...
    const user = await UserRepository.activeAccount({ email: otpExist.email });

    delete user.password;

    return new OK(user, 'Active account successfully!');
  };
